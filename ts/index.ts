...
document.addEventListener('DOMContentLoaded', () => {
    const linkChangerButtons = document.querySelectorAll('.kjllj') as NodeListOf<HTMLButtonElement>;
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const menuDivs: NodeListOf<HTMLDivElement> = document.querySelectorAll('.list1 button');

    // 在上述声明的menuDivs数组中，为每个按钮添加点击事件监听器
    menuDivs.forEach((button: HTMLDivElement) => {
        // 当点击按钮时，执行以下代码
        button.addEventListener('click', function () {
            // 声明变量contentId,这个变量的id文本中,替换menu为content
            const contentId: string = this.id.replace('dz', 'hx');

            // 隐藏所有内容
            // 声明一个变量allContents,这个变量的值是获取所有class为content的元素生成的一个NodeList数组.
            const allContents: NodeListOf<HTMLDivElement> = document.querySelectorAll('.none');

            // 循环遍历数组，隐藏所有内容
            allContents.forEach((content: HTMLDivElement) => {
                content.classList.remove('flex');
            });

            // 显示对应的内容
            // 声明变量activeContent,初始化为全文中ID为contentId当前值的元素
            const activeContent: HTMLElement | null = document.getElementById(contentId);
...
(document.querySelectorAll('.list2') as NodeListOf<HTMLElement>).forEach((div: HTMLElement) => {
    div.addEventListener('click', function () {
        // 类型断言，因为我们确信querySelector的结果是一个HTMLElement
        const textToCopy = (this.querySelector('.kjllj') as HTMLElement).textContent;

        // 类型断言，因为我们确信querySelector的结果是一个HTMLElement
        const houseTypeNameDiv = document.querySelector('.hxxx') as HTMLElement;
